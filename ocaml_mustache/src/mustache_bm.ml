let rec do_tests (template : string) (bindings : Ezjsonm.t) (num : int) : unit =
  if num == 0 then ()
  else
    begin
      Mustache.render (Mustache.of_string template) bindings
      |> ignore;
...
let () =
  ...
  | [| _ ; template_file; bindings_file; num_tests |] ->
      let template = load_file template_file in
